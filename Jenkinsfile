pipeline {
  ...
  environment {
    dockerimagename = "adil22/go-app:${env.BUILD_NUMBER}"
    dockerImage = ""
  ...
  stages {
        ...
        script {
          // Bind the GitHub credentials using withCredentials block - same for github
            withCredentials([usernamePassword(credentialsId: 'gitlab', usernameVariable: 'GITLAB_USERNAME', passwordVariable: 'GITLAB_TOKEN')]) {
              git credentialsId: 'gitlab', branch: 'main', url: 'http://$PAT@gitlab.example.com:8090/root/go-pipeline.git'
            }
          }
        } 
      } 
    ...
    stage ('Deploy on kubernetes') {
      ...
      steps {
        script{
          sh "ansible-playbook -v playbookMK8s.yaml --extra-vars \"image_id=${dockerimagename}\""
        }
      }
    }
  }
